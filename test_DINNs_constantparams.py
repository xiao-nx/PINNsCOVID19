...
class DINN(nn.Module):
    def __init__(self, t, S_data, I_data, D_data, R_data): #[t,S,I,D,R]
        ...
        self.params = itertools.chain(self.net_sird.parameters(), (self.beta_tilda, self.gamma_tilda, self.mu_tilda))
    ...
    class Net_sird(nn.Module): # input = [[t1], [t2]...[t100]] -- that is, a batch of timesteps 
        ...
        def forward(self, t_batch):
            # 换一下activation function
            sird=F.relu(self.fc1(t_batch))
            sird=F.relu(self.fc2(sird))
            sird=F.relu(self.fc3(sird))
            sird=F.relu(self.fc4(sird))
            # sird=F.relu(self.fc5(sird))
            # sird=F.relu(self.fc6(sird))
            # sird=F.relu(self.fc7(sird))
            # sird=F.relu(self.fc8(sird))
            sird=self.out(sird)
            return sird
    ...
    def net_f(self, t_batch):
            ...
            f1_hat = S_hat_t - (-(self.beta / self.N) * S * I)  / (self.S_max - self.S_min)
            f2_hat = I_hat_t - ((self.beta / self.N) * S * I - self.gamma * I - self.mu * I ) / (self.I_max - self.I_min)
    ...
    def train(self, n_epochs):
        ...
        for epoch in range(n_epochs):
            ...
            if epoch % 1000 == 0:          
                ...
                print('beta: (goal 0.25 ', self.beta)
                print('gamma: (goal 0.05 ', self.gamma)
                print('mu: (goal 0.003 ', self.mu)
